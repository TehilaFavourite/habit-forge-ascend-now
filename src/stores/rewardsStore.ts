    ...
    (set, get) => ({
      ...
      claimReward: (id) => {
        set(state => ({
          rewards: state.rewards.map(reward => 
            reward.id === id 
              ? { ...reward, unlocked: true, claimedAt: new Date().toISOString() }
              : reward
          )
        }));
      },
      ...
      getNextReward: (userId, currentXP) => {
        ...
        return userRewards.find(reward => !reward.unlocked && reward.xpRequired > currentXP) || null;
      },
      
      getCurrentLevel: (userId, currentXP) => {
        const userRewards = get().getRewardsForUser(userId);
        const unlockedRewards = userRewards.filter(reward => reward.xpRequired <= currentXP);
        return unlockedRewards.length > 0 ? Math.max(...unlockedRewards.map(r => r.level)) : 1;
      },
    }),
    {
