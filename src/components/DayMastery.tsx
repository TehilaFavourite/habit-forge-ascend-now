...
export const DayMastery = () => {
  ...
  // Day Mastery Requirements
  const morningRoutineHabit = habits.find(h => h.name.toLowerCase().includes('morning'));
  const eveningRoutineHabit = habits.find(h => h.name.toLowerCase().includes('evening'));
  const coreHabits = habits.filter(h => h.isCore);
  const coreXPActivities = xpActivities.filter(a => a.type === 'core');
  
  // Check completions
  const isMorningRoutineComplete = morningRoutineHabit?.completions[selectedDate] || false;
  const isEveningRoutineComplete = eveningRoutineHabit?.completions[selectedDate] || false;
  const completedCoreHabits = coreHabits.filter(h => h.completions[selectedDate]).length;
  ...
  const requirements = [
    {
      ...
      completed: isMorningRoutineComplete,
      required: 1,
      current: isMorningRoutineComplete ? 1 : 0,
    },
    {
      id: 'evening',
      name: 'Evening Routine',
      completed: isEveningRoutineComplete,
      required: 1,
      current: isEveningRoutineComplete ? 1 : 0,
    },
    {
      id: 'habits',
      name: 'Core Daily Habits',
      completed: completedCoreHabits === coreHabits.length && coreHabits.length > 0,
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-start">
        <div>
          ...
          <p className="text-gray-600 mt-1">Complete all requirements to master your day</p>
        </div>
        ...
        <CardHeader>
          ...
          <CardDescription>
            {isDayMastered 
              ...
              : `Complete ${totalRequirements - completedRequirements} more requirement${totalRequirements - completedRequirements !== 1 ? 's' : ''} to master this day`
            }
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            <div className="flex justify-between text-sm mb-2">
              ...
              <span>{completedRequirements}/{totalRequirements} completed</span>
            </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Complete all requirements to achieve day mastery</CardDescription>
        </CardHeader>
      ...
      {/* Setup Instructions */}
      {(coreHabits.length === 0 || !morningRoutineHabit || !eveningRoutineHabit || coreXPActivities.length === 0) && (
        <Card className="border-amber-200 bg-amber-50">
          ...
          <CardContent className="space-y-2 text-sm">
            {!morningRoutineHabit && (
              <div className="text-amber-700">• Create a habit with "morning" in the name</div>
            )}
            {!eveningRoutineHabit && (
              <div className="text-amber-700">• Create a habit with "evening" in the name</div>
            )}
            {coreHabits.length === 0 && (
              <div className="text-amber-700">• Mark some habits as "core" habits</div>
            )}
            {coreXPActivities.length === 0 && (
              <div className="text-amber-700">• Create some core XP activities</div>
            )}
          </CardContent>
        </Card>
      )}
