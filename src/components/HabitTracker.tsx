...
export const HabitTracker = () => {
  ...
  const todayCompletions = coreHabits.filter(habit => habit.completions[today]);
  const dayMasteryComplete = coreHabits.length > 0 && todayCompletions.length === coreHabits.length;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-start">
        ...
        <div className="flex gap-2">
          {coreHabits.length > 0 && (
            <Button
              onClick={handleCompleteAllCore}
              disabled={dayMasteryComplete}
              className={`${
                dayMasteryComplete 
            ...
            <div className="flex items-center gap-4">
              <div className="flex-1">
                <div className="flex justify-between text-sm mb-2">
                  <span>Progress</span>
                  <span>{todayCompletions.length}/{coreHabits.length}</span>
                </div>
                <div className="w-full h-3 bg-gray-200 rounded-full overflow-hidden">
                  <div 
                    className="h-full bg-gradient-to-r from-purple-500 to-blue-500 transition-all duration-500"
                    style={{ width: `${(todayCompletions.length / coreHabits.length) * 100}%` }}
                  />
                </div>
              </div>
              {dayMasteryComplete && (
      ...
      {habits.length > 0 ? (
        ...
        <Card className="text-center py-12 bg-white/70 backdrop-blur-sm border-0 shadow-lg">
          <CardContent>
            ...
            <p className="text-gray-500 mb-6">Start building your daily routine by creating your first habit</p>
            <Button
              onClick={() => setShowForm(true)}
