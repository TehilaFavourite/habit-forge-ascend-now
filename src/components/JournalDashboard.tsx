...
export const JournalDashboard = () => {
  ...
  const todayEntry = getEntryByDate(
    userId,
  ...
  const handleAddTag = () => {
    if (
      newEntry.newTag.trim() &&
      !newEntry.tags.includes(newEntry.newTag.trim())
    ) {
      setNewEntry((prev) => ({
        ...prev,
          ...
          <Card className='bg-card/90 backdrop-blur-xl border-primary/20 shadow-gentle'>
            <CardContent className='p-6'>
              <div className='flex items-center gap-3'>
                ...
                <div>
                  <p className='text-2xl font-bold text-journal-primary'>
                    {streak}
                  </p>
                        ...
                        filteredEntries.map((entry) => (
                          <Dialog key={entry.id}>
                            <DialogTrigger asChild>
                              <Card className='border-muted/20 hover:border-primary/30 transition-colors cursor-pointer'>
                        ...
                        <CardContent>
                          <div className='space-y-2'>
                            {[
                              ...
                              const count = userEntries.filter(
                                (e) => e.type === type
                              ).length;
                              const percentage =
                                userEntries.length > 0
                                  ? (count / userEntries.length) * 100
                                  : 0;
