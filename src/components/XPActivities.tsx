...
export const XPActivities = () => {
  ...
  const { activities, completions, addActivity, deleteActivity, completeActivity, getActivitiesForUser, getTotalXPForDate, getTotalXPForUser } = useXPStore();
  ...
  const handleCompleteActivity = (activity: XPActivity) => {
    const existingCompletions = completions.filter(c => 
      c.activityId === activity.id && c.date === today
    );
    
    if (activity.type === 'core' && activity.dailyCap) {
      const todayXP = existingCompletions.reduce((sum, c) => sum + c.xpEarned, 0);
  ...
  const ActivityCard = ({ activity }: { activity: XPActivity }) => {
    ...
    const canComplete = activity.type === 'bonus' || !activity.dailyCap || todayXP < activity.dailyCap;
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Badge variant={activity.type === 'core' ? 'default' : 'secondary'}>
                {activity.xp} XP
              </Badge>
              <Button
                variant="ghost"
          ...
          {activity.type === 'core' && activity.dailyCap && (
            <div className="space-y-2">
              <div className="flex justify-between text-xs text-gray-500">
                <span>Today: {todayXP}/{activity.dailyCap} XP</span>
                <span>{Math.round((todayXP / activity.dailyCap) * 100)}%</span>
              </div>
              <Progress value={(todayXP / activity.dailyCap) * 100} className="h-1" />
            </div>
          )}
        </CardHeader>
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-start">
        <div>
          ...
          <p className="text-gray-600 mt-1">Earn experience points through daily activities</p>
        </div>
        ...
        {coreActivities.length > 0 ? (
          ...
          <Card className="text-center py-12">
            <CardContent>
              ...
              <p className="text-gray-500">No core activities yet. Add some to start earning XP!</p>
            </CardContent>
          </Card>
        )}
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-xl font-semibold">Bonus Activities (Unlimited)</h3>
        </div>
        ...
        {bonusActivities.length > 0 ? (
          ...
          <Card className="text-center py-12">
            <CardContent>
              ...
              <p className="text-gray-500">No bonus activities yet. Add some for unlimited XP opportunities!</p>
            </CardContent>
          </Card>
        )}
      </div>
        ...
        <Dialog open={true} onOpenChange={() => setShowForm(false)}>
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>Create a new activity to earn experience points</DialogDescription>
            </DialogHeader>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  min="1"
                  value={formData.xp}
                  onChange={(e) => setFormData(prev => ({ ...prev, xp: parseInt(e.target.value) || 1 }))}
                />
              </div>
              ...
              {formData.type === 'core' && (
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setFormData(prev => ({ ...prev, dailyCap: parseInt(e.target.value) || 100 }))}
                  />
                </div>
              )}
